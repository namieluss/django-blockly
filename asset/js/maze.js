...
Maze.init = function (blockly) {
    ...
    Maze.mapOffsetLeft_ = -256;
    Maze.mapOffsetTop_ = -80;
    ...
    finishIcon.style.top = Maze.mapOffsetTop_ + Maze.SIZE * (Maze.finish_.y + 0.5) - finishIcon.offsetHeight;
    finishIcon.style.left = Maze.mapOffsetLeft_ + Maze.SIZE * (Maze.finish_.x + 0.5) - finishIcon.offsetWidth / 2;
...
Maze.schedule = function (startPos, endPos) {
    ...
    Maze.displayPegman(startPos[0] + deltas[0], startPos[1] + deltas[1], Maze.constrainDirection16(startPos[2] + deltas[2]));

    Maze.pidList.push(window.setTimeout(function () {
        Maze.displayPegman(startPos[0] + deltas[0] * 2, startPos[1] + deltas[1] * 2, Maze.constrainDirection16(startPos[2] + deltas[2] * 2));
    }, Maze.STEP_SPEED));

    Maze.pidList.push(window.setTimeout(function () {
        Maze.displayPegman(startPos[0] + deltas[0] * 3, startPos[1] + deltas[1] * 3, Maze.constrainDirection16(startPos[2] + deltas[2] * 3));
    }, Maze.STEP_SPEED * 2))
    ;
    Maze.pidList.push(window.setTimeout(function () {
        Maze.displayPegman(endPos[0], endPos[1], Maze.constrainDirection16(endPos[2]));
    }, Maze.STEP_SPEED * 3));
};
...
Maze.scheduleFail = function (forwards) {
    ...
    Maze.displayPegman(Maze.pegmanX + deltaX, Maze.pegmanY + deltaY, direction16);
...
Maze.displayPegman = function (x, y, d) {
    ...
    pegmanIcon.style.top = Maze.mapOffsetTop_ + Maze.SIZE * (y + 0.5) - pegmanIcon.offsetHeight / 2 - 8;
    pegmanIcon.style.left = Maze.mapOffsetLeft_ + Maze.SIZE * (x + 0.5) - pegmanIcon.offsetHeight / 2 + 2;
